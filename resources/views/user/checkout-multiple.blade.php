...
<div class="container-fluid py-5">
    <div style="max-width: 600px; margin: 50px auto; text-align: center; padding: 40px; border-radius: 15px; background: #e6ffed; border: 2px solid #b3f0c4; box-shadow: 0 4px 15px rgba(46, 125, 50, 0.15);">
                ...
                <ul class="list-unstyled text-start">
                    @foreach($createdOrders as $order)
                        <li class="mb-2">
                            ...
                            <span class="text-muted">{{ $order['service_name'] }}</span> — 
                            <span class="text-success fw-bold">{{ number_format($order['total_amount'], 0, ',', '.') }} VNĐ</span>
                        </li>
                    @endforeach
                </ul>
            </div>
        ...
        <div class="mt-4">
            <p class="text-muted mb-3">Chuyển hướng đến giỏ hàng trong 5 giây...</p>
            <a href="{{ route('user.cart') }}" class="btn btn-success btn-lg px-5">
                <i class="bi bi-bag-check"></i> Về giỏ hàng
            </a>
        </div>
