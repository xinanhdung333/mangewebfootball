...
<div class="card shadow-sm border-0">
    <div class="card-body pb-0">
        @include('partials.admin-table-search', ['tableId' => 'admin-categories-table', 'placeholder' => 'Tìm theo tên danh mục...'])
    </div>
    <div class="table-responsive">
        <table id="admin-categories-table" class="table table-hover align-middle mb-0">
            ...
            @forelse($categories as $category)
                <tr>
                    ...
                    <td class="text-end pe-3">
                        ...
                        @if($category->id !== 1)
                            <form method="POST" action="{{ route('admin.delete.category') }}" class="d-inline">@csrf<input type="hidden" name="id" value="{{ $category->id }}"><button class="btn btn-sm btn-outline-danger" onclick="return confirm('Xóa danh mục này? Sản phẩm sẽ chuyển về Tổng hợp.')"><i class="bi bi-trash"></i> Xóa</button></form>
                        @endif
                    </td>
                </tr>
            @empty
...
@include('partials.category-modals', ['prefix' => 'admin'])
@endsection
