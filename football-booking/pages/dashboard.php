    ...
    $stats_total = $conn->query("SELECT COUNT(*) FROM bookings WHERE user_id = {$_SESSION['user_id']}")->fetch_row()[0];
    $stats_confirmed = $conn->query("SELECT COUNT(*) FROM bookings WHERE user_id = {$_SESSION['user_id']} AND status='confirmed'")->fetch_row()[0];
    $stats_revenue = $conn->query("SELECT SUM(total_price) FROM bookings WHERE user_id = {$_SESSION['user_id']} AND status='confirmed'")->fetch_row()[0] ?? 0;
...
.between-img:hover {
    ...
    <section class="hero-modern">
        <div class="hero-content-modern">
            <h1 class="hero-title">Chào mừng <?php echo htmlspecialchars($user['name']); ?> — Đặt sân nhanh, chơi đã</h1>
